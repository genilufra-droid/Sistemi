...
import { fmt, partyDue, useStore } from "@/lib/store";
...
export default function PartiesScreen() {
  ...
  const list = db.customers.filter((c) => c.name.toLowerCase().includes(q.toLowerCase()));

  return (
    <ScreenContainer>
      <Header title="Klientet" back={() => router.back()} />
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          const due = partyDue(db, item.name);
          return (
            <Pressable
      ...
      />
      <Pressable
        onPress={() => router.push({ pathname: "/party-form", params: { kind: "customer" } } as any)}
        style={({ pressed }) => ({
          position: "absolute",
          bottom: 20,
          right: 20,
